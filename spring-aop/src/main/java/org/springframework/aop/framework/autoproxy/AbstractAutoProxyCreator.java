...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
    ...
    public Object postProcessAfterInitialization(Object bean, String beanName) throws BeansException {
        if (bean != null) {
            // 根据给定Bean的 Class & Name 构建一个Key，格式为：beanClassName_beanName
            Object cacheKey = getCacheKey(bean.getClass(), beanName);
            if (!this.earlyProxyReferences.contains(cacheKey)) {
    ...
    protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
        ...
        // 之前已经判断过一次，结果是不需要创建代理，所以第二次及以后不再需要判断
        if (Boolean.FALSE.equals(this.advisedBeans.get(cacheKey))) {
            return bean;
        }
        // 给定的Bean类是否是一个基础设施类（基础设施类不应该被代理），或者配置中指定了Bean不需要被代理
    ...
    protected boolean isInfrastructureClass(Class<?> beanClass) {
        boolean retVal = Advice.class.isAssignableFrom(beanClass) || Advisor.class.isAssignableFrom(beanClass)
            ...
            for (TargetSourceCreator tsc : this.customTargetSourceCreators) {
                ...
                if (ts != null) {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug("TargetSourceCreator [" + tsc + " found custom TargetSource for bean with name '"
                                + beanName + "'");
                    }
                    return ts;
                }
