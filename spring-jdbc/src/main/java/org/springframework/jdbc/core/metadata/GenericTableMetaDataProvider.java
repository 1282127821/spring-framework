...
public class GenericTableMetaDataProvider implements TableMetaDataProvider {
     ...
     */
    private void locateTableAndProcessMetaData(DatabaseMetaData databaseMetaData, String catalogName, String schemaName,
            String tableName) {
    ...
    private void processTableColumns(DatabaseMetaData databaseMetaData, TableMetaData tmd) {
        ...
        try {
            tableColumns =
                    databaseMetaData.getColumns(metaDataCatalogName, metaDataSchemaName, metaDataTableName, null);
            while (tableColumns.next()) {
                ...
                if (dataType == Types.DECIMAL) {
                    ...
                    if ("NUMBER".equals(typeName) && decimalDigits == 0) {
                        ...
                        if (logger.isDebugEnabled()) {
                            logger.debug(
                                    "Overriding metadata: " + columnName + " now using NUMERIC instead of DECIMAL");
                        }
                    }
                }
