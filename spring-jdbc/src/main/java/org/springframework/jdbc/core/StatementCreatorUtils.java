...
public abstract class StatementCreatorUtils {
        ...
        if (sqlType == SqlTypeValue.TYPE_UNKNOWN || sqlType == Types.OTHER) {
            ...
            if (checkGetParameterType) {
                try {
                    ...
                    if (logger.isDebugEnabled()) {
                        logger.debug(
                                "JDBC 3.0 getParameterType call not supported - using fallback method instead: " + ex);
                    }
                }
            }
    ...
    private static void setValue(PreparedStatement ps, int paramIndex, int sqlType, String typeName, Integer scale,
        ...
        } else if (sqlType == Types.VARCHAR || sqlType == Types.NVARCHAR || sqlType == Types.LONGVARCHAR
