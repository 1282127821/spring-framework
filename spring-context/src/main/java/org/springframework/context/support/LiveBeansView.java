            ...
            synchronized (applicationContexts) {
                if (applicationContexts.isEmpty()) {
                    try {
                        ...
                        server.registerMBean(new LiveBeansView(), new ObjectName(mbeanDomain, MBEAN_APPLICATION_KEY,
        ...
        synchronized (applicationContexts) {
            if (applicationContexts.remove(applicationContext) && applicationContexts.isEmpty()) {
                try {
                    MBeanServer server = ManagementFactory.getPlatformMBeanServer();
                    String mbeanDomain = applicationContext.getEnvironment().getProperty(MBEAN_DOMAIN_PROPERTY_NAME);
                    server.unregisterMBean(new ObjectName(mbeanDomain, MBEAN_APPLICATION_KEY,
                            applicationContext.getApplicationName()));
                } catch (Exception ex) {
