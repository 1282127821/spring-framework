...
public class JRubyScriptFactory implements ScriptFactory, BeanClassLoaderAware {
    ...
    public Object getScriptedObject(ScriptSource scriptSource, Class<?>... actualInterfaces)
        ...
        try {
            return JRubyScriptUtils.createJRubyObject(scriptSource.getScriptAsString(), actualInterfaces,
                    this.beanClassLoader);
        } catch (RaiseException ex) {
            RubyException rubyEx = ex.getException();
            String msg =
                    (rubyEx != null && rubyEx.message != null) ? rubyEx.message.toString() : "Unexpected JRuby error";
